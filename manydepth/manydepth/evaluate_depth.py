...
from .layers import transformation_from_parameters, disp_to_depth, invert_pose
import tqdm
...
def compute_errors(gt, pred):
    ...
    a1 = (thresh < 1.25).astype(float).mean()
    a2 = (thresh < 1.25 ** 2).astype(float).mean()
    a3 = (thresh < 1.25 ** 3).astype(float).mean()
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        if opt.eval_split == 'cityscapes':
            dataset = datasets.CityscapesEvalDataset(opt.data_path, filenames,
                                                     ...
                                                     frames_to_load, 4,
                                                     is_train=False)
        elif opt.eval_split == 'm4depth-MidAir':
            dataset = datasets.MidAirDataset(opt.data_path, filenames,
                                                     HEIGHT, WIDTH,
                                                     frames_to_load, 4,
                                                     is_train=False)
        elif opt.eval_split == "m4depth-tartanair-urban":
            opt.eval_split = "m4depth-tartanair"
            dataset = datasets.TartanAirDataset(opt.data_path, filenames,
                                                     HEIGHT, WIDTH,
                                                     frames_to_load, 4,
                                                     is_train=False)
        elif opt.eval_split == "m4depth-tartanair-unstructured":
            opt.eval_split = "m4depth-tartanair"
            dataset = datasets.TartanAirDataset(opt.data_path, filenames,
                                                     HEIGHT, WIDTH,
                                                     frames_to_load, 4,
        ...
        frames_to_load = dataset.frame_idxs
            ...
            for i, data in tqdm.tqdm(enumerate(dataloader)):
                ...
                if opt.eval_teacher:
                    ...
                    for fi in frames_to_load[1:]:
                        # Use groundtruth positioning if available
                        if ("poses", fi) in data:
                            pose = invert_pose(torch.matmul(invert_pose(data[("poses", 0)]) , data[("poses", fi)]))
                        else:
                            if fi < 0:
                        ...
                        data[('relative_pose', fi)] = pose.type(torch.FloatTensor)
    ...
    if opt.eval_split == 'cityscapes':
        ...
        dataset = datasets.MidAirDataset(opt.data_path, filenames,
                                         ...
                                         frames_to_load, 4,
                                         is_train=False)
    elif opt.eval_split == 'm4depth-tartanair':
        dataset = datasets.TartanAirDataset(opt.data_path, filenames,
                                         HEIGHT, WIDTH,
                                         frames_to_load, 4,
                                         is_train=False)
    ...
    for i in tqdm.tqdm(range(pred_disps.shape[0])):
        ...
        elif opt.eval_split == 'm4depth-MidAir' or opt.eval_split == 'm4depth-tartanair':
            gt_depth = dataset[i]["depth_gt"]
            gt_height, gt_width = gt_depth.shape[:2]
        ...
        if opt.export_pics and not (i+1)%8:
            save_maps(np.array(gt_depth), np.array(pred_depth), i)
        ...
        # print(mask.shape)
        # print(np.array(pred_depth)[mask].shape)
        pred_depth = np.array(pred_depth)[mask]
        gt_depth = np.array(gt_depth)[mask]
