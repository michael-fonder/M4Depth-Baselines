...
# def samples_writer(serializer, data_queue, shards_queue, terminate_event):
#     print('test')
#
#     def _jpeg2feature(sess, image_data):
#         """Wrapper for inserting jpeg images into Example proto."""
#         if type(image_data) is str:
#             bytes_jpg = _bytes_feature(tf.compat.as_bytes(open(image_data, 'rb').read()))
#         else:
...
#             bytes_jpg = _bytes_feature(tf.compat.as_bytes(encoded_jpeg))
#         return bytes_jpg
#
#     def _png2feature(sess, image_data):
#         """Wrapper for inserting png images into Example proto."""
#         if type(image_data) is str:
#             bytes_png = _bytes_feature(tf.compat.as_bytes(open(image_data, 'rb').read()))
#         else:
...
#             bytes_png = _bytes_feature(tf.compat.as_bytes(encoded_png))
#         return bytes_png
#
#     def _int64_feature(sess, value):
#         """Wrapper for inserting int64 features into Example proto."""
#         if not isinstance(value, list):
#             value = [value]
#         return tf.train.Feature(int64_list=tf.train.Int64List(value=value))
#
#     def _float32_feature(sess, value):
#         """Wrapper for inserting Float32 features into Example proto."""
#         if not isinstance(value, list):
#             value = [value]
#         return tf.train.Feature(float_list=tf.train.FloatList(value=value))
#
#     def _bytes_feature(sess, value):
#         """Wrapper for inserting bytes features into Example proto."""
#         return tf.train.Feature(bytes_list=tf.train.BytesList(value=[value]))
#
#     def _generic2feature(sess, string):
#         return _bytes_feature(tf.compat.as_bytes(string))
#
#
#     try:
#         print('test')
...
#             "int64_list": _int64_feature,
#             "float32_list": _float32_feature,
#             "string": _generic2feature,
#             "jpeg": _jpeg2feature,
#             "png": _png2feature,
#             "bytes": _generic2feature,
#         }
#
#         writer = None
#         sess = tf.Session()
...
#                 output_filename = '-%.5d' % (shard_id)
#                 output_file = data_prefix + output_filename
...
#                 sample_dict[name] = name2var_dict[var_type](sess, sample[i])
#
...
class ProtoBufSerializer:
    ...
    def __init__(self, features_list, data_dir = "serialized_DB", shard_prefix = "serialized_data_shard", samples_per_shard = 512, jpeg_q = 90, nbre_threads=4):
        ...
        #     # workers.apply_async(func=samples_writer,
        #     #                      args=([self], None, None, None))
        #
            process = mp.Process(target=self.samples_writer,
                                 args=( self.data_queue, self.shards_queue, self.terminate_event))
            # process = mp.Process(target=unwrap_self_samples_writer,
            #                      args=(weakref.ref(self), self.data_queue, self.shards_queue, weakref.ref(self.terminate_event)))
            process.start()
    ...
    def process_sample(self, single_sample):
        ...
        #     self.sess = tf.Session()
        #
        # if len(single_sample) != len(self.features_list):
        #     raise Exception("Number of features mismatch")
        #
        # if self.cnter%self.samples_per_shard == 0:
        #     #create new shard
        #     shard = int(self.cnter/self.samples_per_shard)
        #     output_filename = '%s-%.5d' % (self.shard_prefix, shard)
        #     output_file = os.path.join(self.data_dir, output_filename)
        #     self.writer = tf.python_io.TFRecordWriter(output_file)
        #
        # #write into file
        # sample_dict = {}
        # for i, (name, var_type) in enumerate(self.features_list):
        #     sample_dict[name] = self.name2var_dict[var_type](single_sample[i])
        #
        # sample_example = tf.train.Example(features = tf.train.Features(feature = sample_dict))
        # self.writer.write(sample_example.SerializeToString())
        # self.cnter += 1
        #
        # if self.cnter%self.samples_per_shard == 0:
        #     # End of shard => close file before creating a new one
        #     self.writer.close()
        #
        #     shard = int(self.cnter/self.samples_per_shard)-1
        #     print('%s : Wrote %d images to %s-%.5d' %
        #     (datetime.now(), self.samples_per_shard, self.shard_prefix, shard))
    ...
    def end_serializing(self):
        ...
        #     self.writer.close()
        # print('%s : Wrote %d images to %s-%.5d' %
        #     (datetime.now(), (self.cnter-1)%self.samples_per_shard, self.shard_prefix, self.cnter//self.samples_per_shard))
...
class ProtoBufDeserializer:
    ...
    def __init__(self, features_list, sess = None, data_dir = "serialized_DB", shard_prefix = "serialized_data_shard", samples_per_shard = 512):
        ...
        self.name2deserializer_dict = {
            "int64" : tf.FixedLenFeature([1], tf.int64,  -1),
            "float32" : tf.FixedLenFeature([1], tf.float32,  -1),
            "int64_list" : tf.VarLenFeature(dtype=tf.int64),
            "float32_list" : tf.VarLenFeature(dtype=tf.float32),
            "string" : tf.FixedLenFeature([ ], tf.string, ''),
            "jpeg" : tf.FixedLenFeature([ ], tf.string, ''),
            "png" : tf.FixedLenFeature([ ], tf.string, ''),
            "png16" : tf.FixedLenFeature([ ], tf.string, ''),
            "float16_mat" : tf.FixedLenFeature([ ], tf.string, ''),
    ...
    def deserialize_record(self, record):
        ...
        with tf.name_scope('deserialize_image_record'):
            obj = tf.parse_single_example(record, self.feature_map)
            record_data = []
