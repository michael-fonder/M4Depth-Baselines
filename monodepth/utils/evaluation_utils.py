...
def compute_errors(gt, pred):
    ...
    print("{:10.4f}, {:10.4f}, {:10.3f}, {:10.3f}".format(abs_rel.mean(), sq_rel.mean(), rmse.mean(), rmse_log.mean()))
...
def load_gt_disp_kitti(path):
    gt_disparities = []
    print("Reading : %s" % os.path.join(path, "depth_samples.txt"))
    with open(os.path.join(path, "depth_samples.txt"), 'r') as csvfile:
        csv_log = csv.reader(csvfile, delimiter=' ')
        for gt_path in csv_log:

            print(gt_path)
            pic = Image.open(path+gt_path[0])
...
def convert_disps_to_depths_kitti(gt_disparities, pred_disparities):
    ...
    for i in range(len(gt_disparities)):
        ...
        gt_depth = gt_disp #width_to_focal[width] * 0.54 / (gt_disp + (1.0 - mask))
        pred_depth = 512 * 1 / (pred_disp)

        #convert planar to real depth
        angles = np.linspace(-np.pi/4, np.pi/4, width)
        factor = 1/(np.sin(np.pi/2 - np.abs(angles)))
        grid = np.meshgrid(factor, factor)
        pred_depth *= grid[0]*grid[1]
