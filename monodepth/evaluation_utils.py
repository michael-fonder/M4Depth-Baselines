...
def load_gt_depth_tartanair(db_path, path):
    gt_depth = []
    print("Loading tartanair gt depth")
    with open(os.path.join(path, "disp_samples.txt"), 'r') as csvfile, open(os.path.join(path, "stereo_samples.txt"), 'r') as colorfile:
        csv_log = csv.reader(csvfile, delimiter=' ')
        color_log = csv.reader(colorfile, delimiter=' ')
        for i, (gt_path, col_path) in enumerate(zip(csv_log, color_log)):
            if not i%500:
                print("%i samples loaded" % i)

            depth = np.load(os.path.join(db_path,gt_path[0]))
            depth = Image.fromarray(depth).resize([512,384], Image.NEAREST)
            depth = np.asarray(depth, np.float32)
            color = Image.open(os.path.join(db_path,col_path[0])).resize([512,384], Image.NEAREST)

            greyscale = np.linalg.norm(color, axis=-1)
            mask = np.where(greyscale > 0, 1., 0.)
            gt_depth.append(depth*mask)
    return gt_depth

def load_gt_disp_midair(db_path, path):
    gt_disparities = []
    print("test***************************************************************")
    with open(os.path.join(path, "disp_samples.txt"), 'r') as csvfile:
        csv_log = csv.reader(csvfile, delimiter=' ')
        for i, (gt_path) in enumerate(csv_log):
            if not i%500:
                print("%i samples loaded" % i)
            pic = Image.open(os.path.join(db_path,gt_path[0])).resize([384, 384], Image.NEAREST)
            img = np.asarray(pic, np.uint16)
            img.dtype = np.float16
            img = img.astype(np.float32)
            gt_disparities.append(img)
    return gt_disparities


def convert_disps_to_depths_midair(gt_disparities, pred_disparities):
    gt_depths = []
    pred_depths = []
    ...
    for i in range(len(gt_disparities)):
        ...
        gt_depth = 512. / (gt_disp+0.000001) #width_to_focal[width] * 0.54 / (gt_disp + (1.0 - mask))
        pred_depth = 192. * 1. / (pred_disp+0.000001)
