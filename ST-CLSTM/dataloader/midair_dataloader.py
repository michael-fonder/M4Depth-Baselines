...
class depthDataset(Dataset):
    ...
    def __init__(self, dict_dir, root_dir, img_size = [384,384], transform=None, is_test=False):
    ...
    def depth_video_loader(self, root_dir, index, rand_offset):
        ...
        for i in range(self.seq_len):
            ...
            if os.path.exists(image_path):
                if self.is_test:
                    disp_gt = open_float16(image_path, self.height, self.width)
                else:
                    disp_gt = open_float16(image_path, self.height//2, self.width//2)
...
def getTrainingData_MidAir(batch_size=64, dict_dir=None, root_dir=None, img_size = [384,384]):
    __imagenet_pca = {
...
def getTestingData_MidAir(batch_size=64, dict_dir=None, root_dir=None, num_workers=4, img_size = [384,384]):
