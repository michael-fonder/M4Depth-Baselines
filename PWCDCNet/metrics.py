...
def masked_reduce_mean(err, gt_depth):
    mask = tf.cast(tf.greater(gt_depth, 0.), tf.float32)
...
class RootMeanSquaredLogError(tf.keras.metrics.Mean):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        error = tf.square(y_true-y_pred)
        error = tf.reduce_sum(tf.math.multiply_no_nan(error,mask))/tf.reduce_sum(mask)

        return super(RootMeanSquaredLogError, self).update_state(tf.sqrt(error))
...
class AbsRelError(tf.keras.metrics.Mean):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        error = tf.math.abs(y_true - y_pred) / y_true
...
class SqRelError(tf.keras.metrics.Mean):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        error = tf.math.squared_difference(y_true, y_pred) / y_true
        error = masked_reduce_mean(error, y_true)
